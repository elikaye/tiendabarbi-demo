...
function normalizarSubcategoria(subRaw){
  ...
  if(sub.includes("body") || sub.includes("bodies")) return "bodys";
  if(sub.includes("enterito")) return "enteritos";
  if(sub.includes("conjunto")) return "conjuntos";
  if(sub.includes("abrigo")) return "abrigos";
  return "otros";
}
  ...
  const fetchProductos = async()=>{
    ...
    try{
      ...
      const prods = (data.products || []).map(p=>({
        ...p,
        id:p.id||p._id,
        categoria: p.categoria?.toLowerCase() === "bebes" ? "bebes" : "otros",
      ...
      rawRef.current = prods.filter(p=>{
        if(p.categoria !== "bebes") return false;
      ...
      console.error(err);
      setError("No se pudieron cargar los productos de bebés.");
      setProductos([]);
    }finally{
      setLoading(false);
    }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-10 text-center">
          <h1 className="text-3xl font-bold text-gray-800">
            {tituloSubcategoria ? tituloSubcategoria : "Bebés"}
          </h1>
        </div>
