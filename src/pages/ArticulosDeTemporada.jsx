...
function normalizarCategoria(catRaw){
  ...
  if(cat.includes("tempor") || cat.includes("nav") || cat.includes("verano") || cat.includes("invie")) return "temporada";
  return "otros";
}
...
export default function ArticulosDeTemporada(){
  ...
  const fetchProductos=async()=>{
    ...
    try{
      ...
      rawRef.current = prods.filter(p=>p.categoria==="temporada");
      setProductos(mezclarBalanceado(rawRef.current));
      setError(null);
    }catch(err){ console.error(err); setError("No se pudieron cargar los productos de temporada."); setProductos([]);}
  ...
  useEffect(()=>{
    ...
    try{ if(io){ socketClient=io.connect(window.location.origin); socketRef.current=socketClient; socketClient.on("productos:changed",fetchProductos);} }catch{}
  ...
  return (
    <section className="min-h-screen py-20 px-6 bg-gradient-to-br from-pink-100 via-white to-pink-200 font-body font-semibold">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-2xl font-body font-semibold mb-6 text-black drop-shadow-sm"></h2>

        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {Array(COLUMNAS_MOBILE*3).fill(0).map((_,i)=><div key={i} className="bg-gray-200 rounded-xl h-64 animate-pulse"></div>)}
          </div>
        ) : error ? (
