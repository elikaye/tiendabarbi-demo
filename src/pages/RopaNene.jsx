...
function normalizarSubcategoria(subRaw){
  ...
  if(sub.includes("pantalon")) return "pantalones";
  if(sub.includes("campera") || sub.includes("buzo")) return "camperas y buzos";
  if(sub.includes("short") || sub.includes("bermuda")) return "shorts y bermudas";
...
export default function RopaNene(){
  ...
  const fetchProductos = async()=>{
    ...
    try{
      ...
      const prods = (data.products || []).map(p=>({
        ...p,
        id:p.id||p._id,
        categoria: p.categoria?.toLowerCase() === "ropa de nene" ? "ropa de nene" : "otros",
      ...
      rawRef.current = prods.filter(p=>{
        if(p.categoria !== "ropa de nene") return false;
      ...
      console.error(err);
      setError("No se pudieron cargar los productos de ropa de nene.");
      setProductos([]);
    }finally{
      setLoading(false);
    }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-10 text-center">
          <h1 className="text-3xl font-bold text-gray-800">
            {tituloSubcategoria ? tituloSubcategoria : "Ropa de Nene"}
          </h1>
        </div>
